...
TRANSCRIPT_PATCH="$ROOT/patches/stwo-transcript-sink.patch"
...
REQUIRED_TRANSCRIPT_PATCH_SHA256="da841076b861f00a1a128d5aaa49f092a36809d17bf43520fb25f1dafd2d1746"
REQUIRED_SOURCE_ID="stwo@$REQUIRED_COMMIT+patches:accessor=$REQUIRED_ACCESSOR_PATCH_SHA256;consumption=$REQUIRED_CONSUMPTION_PATCH_SHA256;transcript=$REQUIRED_TRANSCRIPT_PATCH_SHA256"
...
TMP_SINK_INDEX="$(mktemp "${TMPDIR:-/tmp}/airlock-stwo-sink-index.XXXXXX")"
trap 'rm -f "$TMP_ACCESSOR_PATCH" "$TMP_SINK_INDEX"' EXIT
...
[[ -f "$TRANSCRIPT_PATCH" ]] || fail "missing checked transcript patch at $TRANSCRIPT_PATCH"
...
[[ "$("${SHA256[@]}" "$TRANSCRIPT_PATCH" | awk '{print $1}')" == "$REQUIRED_TRANSCRIPT_PATCH_SHA256" ]] ||
  fail "transcript patch SHA-256 does not match the bound source identity"
...
GIT_INDEX_FILE="$TMP_SINK_INDEX" git -C "$STWO_DIR" read-tree "$REQUIRED_COMMIT"
# Both sink patches edit the same files, so the working tree must equal the
# accumulated result rather than either patch alone. Apply them in the order
# scripts/setup-stwo.sh applies them.
for patch in "$CONSUMPTION_PATCH" "$TRANSCRIPT_PATCH"; do
  GIT_INDEX_FILE="$TMP_SINK_INDEX" git -C "$STWO_DIR" apply --cached --check "$patch"
  GIT_INDEX_FILE="$TMP_SINK_INDEX" git -C "$STWO_DIR" apply --cached "$patch"
done
for path in "${SINK_TARGETS[@]}"; do
  expected_object="$(
    GIT_INDEX_FILE="$TMP_SINK_INDEX" git -C "$STWO_DIR" rev-parse ":$path"
  )"
  actual_object="$(git -C "$STWO_DIR" hash-object "$STWO_DIR/$path")"
  [[ "$actual_object" == "$expected_object" ]] || fail \
    "Stwo sink target $path does not equal the checked accessor, consumption, and transcript patch result"
done

printf 'PASS: Stwo dependency and held-out target sources at %s match upstream baseline %s plus the checked accessor, consumption, and transcript patches\n' \
